...
if __name__ == '__main__':
	...
	for i in range(7):
		...
		dmt.plot(ax=axes[i],y='tickets', x='start_time_y',kind='bar', legend=False,fontsize=6)
		#         dmt.plot(ax=axes[0,0]); axes[0,0].set_title('A')

		axes[i].set_xlabel(u'时 间(2016-12-'+str(21)+u' 06:00—23:00)',size=10)
		axes[i].set_ylabel(u'上下客流量值（单位：人）',size=10)
		axes[i].set_title(u'苏州地区站点（上海——南京）\n',size=10)
	plt.tight_layout()
	plt.savefig('suzhou_full.png',dpi=300)
	plt.show()
	print('finished!')